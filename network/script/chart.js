export const networkGraph = (
  { nodes: allnodes, edges: alledges },
  {
    ...
    accessorNodeDept,
    accessorNodeCount,
    accessorNodefullname,
    accessorLinkSourceId,
    accessorLinkTargetId,
    matrixColorScheme = ['rgb(46, 73, 123)', 'rgb(71, 187, 94)'],
    matrixBackGroundColor = '#c8dcde',
    nodeStroke = '#fff',
    nodeStrokeWidth = 1.5,
    nodeStrokeOpacity = 1,
    linkStroke = '#999',
  ...
  const scalerColorMatrixGraph = d3
    ...
    .range(matrixColorScheme)

  const background = matrixGraph
    .append('rect')
    .attr('fill', matrixBackGroundColor)
    .attr('width', matrixSize.width)
    .attr('height', matrixSize.height)
  ...
  function rowPainter(row) {
    var cell = d3
      ...
      .data(
        row.filter(
          (d) => d.z, // Select only nonzero->True elements
        ),
      )
      .enter()
      .append('rect')
      ...
      .style('fill', (d) => scalerColorMatrixGraph(d.z))
  ...
  const node = nodeLinkGraph
    ...
    .data(nodes)
    .join('circle')
    .attr('fill', (d) => nodeColorScaler(accessorNodeCount(d)))
    .attr('r', 6)
